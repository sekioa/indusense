...
Le travail pratique correspondant est déjà présent dans [`01-maintenance-ml-regression.ipynb`](../../../notebooks/02-sprint-2/01-maintenance-predictive/01-maintenance-ml-regression.ipynb). Cette fiche décrit son déroulement et la lecture des résultats sans modifier le notebook exécuté.
...
- La transcription du cours est disponible dans [`01-jour-1-cours-apres-midi.txt`](../02-transcriptions/01-jour-1-cours-apres-midi.txt). Les notions mobilisées sont notamment la descente de gradient, la normalisation, le sous-apprentissage, le surapprentissage et la régularisation.
...
Dans JupyterLab, ouvrir `notebooks/02-sprint-2/01-maintenance-predictive/01-maintenance-ml-regression.ipynb`, puis exécuter les cellules dans l'ordre avec `Shift + Entrée`. La première cellule doit confirmer le fichier chargé, 134 280 lignes, 15 machines et l'absence de doublon sur `machine_id_std` et l'ancre temporelle.
